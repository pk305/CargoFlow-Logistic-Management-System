...
import { fetchVehicles } from 'src/redux/slices/vehicleSlice'
          ...
          <div className="pageSearchContainer">
            <div className="filter-actions">
              ...
              <div className="filterSelected-text">
                {selectedFlatRows.length === 1 ? 'Company' : 'Vehicles'} Selected
              </div>
...
const Vehicles = () => {
  ...
  const fetchVehicleData = useCallback(() => dispatch(fetchVehicles()), [dispatch])
    ...
    () => [
      {
        ...
        Cell: ({ value, row }) => (
          <div className="link-search-hover">
            <div>
              {/*  eslint-disable-next-line react/prop-types */}
              <Link to={`/vehicles/${row.original.linkId}`}>{value}</Link>
            </div>
          </div>
        ),
      },
      {
        Header: <span>Driver / Staff</span>,
        accessor: 'customerType',
      },
      {
        Header: <span>Vehicle Class</span>,
        accessor: 'email',
      },
      {
      ...
      {
        Header: <span>Owner Company</span>,
        accessor: 'branch',
      },
      {
  ...
  const data = useMemo(() => (vehicles && vehicles.length > 0 ? vehicles : []), [vehicles])
                ...
                <div className="customHeaderContainer">
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="cstCardbodyHeaderTitle">Vehicles</h3>
                    </div>
                  </div>
                </div>
                <div className="customHeaderToolbar">
                  <CButtonGroup>
                    <CButton color="primary" active onClick={(e) => toLink(e, '/vehicles/new')}>
                      <i className="fa fa-plus mr-2" /> Create Vehicle
                    </CButton>
                  </CButtonGroup>
                </div>
              </div>
