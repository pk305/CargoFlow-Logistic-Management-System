...
import { fetchGldocs } from 'src/redux/slices/gldocSlice'
...
const Table = ({ columns, data }) => {
  ...
  const { fetchingGldocs } = useSelector((state) => state.gldoc)
...
const LedgerEntries = () => {
  ...
  const { gldocs } = useSelector((state) => state.gldoc)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'actions',
        Cell: (props) => (
          <div className="actions-items right">
            <div className="d-flex">
              <div>
                <CButton color="link" className="actions-btn">
  ...
  const data = useMemo(() => (gldocs && gldocs.length > 0 ? gldocs : []), [gldocs])

  const fetchGldocData = useCallback(() => dispatch(fetchGldocs()), [dispatch])

  useEffect(() => {
    document.title = 'Ledger Entries'
    //
    fetchGldocData()
  }, [fetchGldocData])
                ...
                <div className="customHeaderContainer">
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="cstCardbodyHeaderTitle">Ledger Entries</h3>
                    </div>
                  </div>
                </div>
                <div className="customHeaderToolbar">
                  <CButtonGroup>
                    <CButton color="primary" active onClick={(e) => toLink(e, '/gldocs/new')}>
                      <i className="fa fa-plus mr-2" /> Create Ledger Entry
                    </CButton>
                  </CButtonGroup>
                </div>
              </div>
