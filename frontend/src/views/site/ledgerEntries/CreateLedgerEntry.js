...
import { clearGldocError, createGldoc, showGldocError } from 'src/redux/slices/gldocSlice'
import $ from 'jquery'
...
const CreateLedgerEntry = () => {
  ...
  const { creatingGldoc, gldocErrors } = useSelector((state) => state.gldoc)
  ...
  const handleSubmitLedger = async (e) => {
    e.preventDefault()
    const form = $('#new_gldoc')
    if (form.length > 0) {
      if (gldocData.title === '') {
        dispatch(showGldocError({ type: 'title', errorType: 'errGldoc' }))
        $('html, body').animate({ scrollTop: 0 }, 300)
        return false
      }
    }
    //form data
    let arrForm = Object.entries(gldocData)
    ...
    const resData = await dispatch(createGldoc(formData)).unwrap()
    if (resData) {
      new Noty({
        type: 'alert',
        layout: 'topRight',
        id: `succ${resData.id}`,
        text: 'Ledger has been created succesfully',
      }).show()
                ...
                <div className="customHeaderContainer">
                  ...
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="cstCardbodyHeaderTitle">New Ledger Entry</h3>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div className="row">
                      <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                        <CButton
                          type="submit"
                          color="success"
                          disabled={creatingGldoc ? true : false}
                        >
                          {creatingGldoc ? (
                            'Processing...'
                          ) : (
