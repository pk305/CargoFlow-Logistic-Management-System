...
// import TusApi from 'src/config/TusApi'
...
const EditCompany = () => {
  ...
  const { updatingCompany, companyErrors } = useSelector((state) => state.company)
      ...
      async function findCompanyData() {
        ...
        if (resData) {
          ...
          setCompanyData((state) => ({
            ...state,
            companyId: res.id,
            companyName: res.name ? res.name : '',
            cityId: res.city && res.city.id ? res.city.id : '',
            countryId: res.countryId ? res.countryId : '',
            district: res.district ? res.district : '',
            address: res.address ? res.address : '',
            postcode: res.postcode ? res.postcode : '',
            eoriCode: res.eoriCode ? res.eoriCode : '',
            taxOffice: res.taxOffice ? res.taxOffice : '',
            taxNo: res.taxno ? res.taxno : '',
            companyTel: res.companyTel ? res.companyTel : '',
            companyFax: res.companyFax ? res.companyFax : '',
            companyEmail: res.email ? res.email : '',
            companyWebsite: res.companyWebsite ? res.companyWebsite : '',
            companySalerId: res.saler && res.saler.id ? res.saler.id : '',
            branchId: res.branch && res.branch ? res.branch.id : '',
            companyGroup: res.companyGroup ? res.companyGroup : '',
            companyType: res.companyType ? res.companyType : '',
            companySector: res.companySector ? res.companySector : '',
            tagNames: res.tagNames ? res.tagNames : '',
            notes: res.notes ? res.notes : '',
            contactsAttrName: res.contactsAttrName ? res.contactsAttrName : '',
            contactsAttrJobTitle: res.contactsAttrJobTitle ? res.contactsAttrJobTitle : '',
            contactsAttrTel: res.contactsAttrTel ? res.contactsAttrTel : '',
            contactsAttrEmail: res.contactsAttrEmail ? res.contactsAttrEmail : '',
          ...
          if (res.city && res.city.id) {
            const countryData = res.city ? countryList().getLabel(res.city.country) : null
            setloadDataValue((state) => ({
              ...state,
              loadCity: res.city ? { label: res.city.name, value: res.city.id } : '',
              loadCountry: countryData
                ? { label: `${res.city.country}-${countryData}`, value: res.city.country }
                : '',
            }))
          }
        }
                ...
                <div className="customHeaderContainer">
                  ...
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="cstCardbodyHeaderTitle">Edit Company</h3>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <form id="edit_company" onSubmit={(e) => handleSubmitCompany(e)}>
                            ...
                            <div className="form-group select company_saler_id">
                              ...
                              <div className="input-group">
                                <Select
                                  ...
                                  id="company_sector"
                                  options={
                                    users && !fetchingCities && users.length > 0
                                      ? users.map((itm) => ({
                                          label: itm.name,
                                          value: itm.id,
                                        }))
                          ...
                          <div className="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                            <div className="form-group select company_branch_id">
                              ...
                              <CFormSelect
                                ...
                                onFocus={(e) => handleSelectFocus('branchId')}
                              >
                                {!fetchingBranches ? (
                                  branches && branches.length > 0 ? (
                                    <>
                                      {branches.map((itm) => (
                                        <option key={itm.uuid} value={itm.uuid}>
                                          {itm.name}
                                        </option>
                                      ))}
                                    </>
                          ...
                          <div className="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                            <div className="form-group select">
                              ...
                              <div className="input-group">
                                ...
                                <CFormFeedback
                                  invalid={
                                    companyErrors && !isEmpty(companyErrors.timezoneId)
                                      ? true
                                      : false
                                  }
                                  className="fieldError-cst d-block"
                                >
