...
import NewIncident from './panels/NewIncident'
import NewServiceLog from './panels/NewServiceLog'
          ...
          <div className="pageSearchContainer">
            <div className="filter-actions">
              ...
              <div className="filterSelected-text">
                {selectedFlatRows.length === 1 ? 'Company' : 'ServiceLogs'} Selected
              </div>
...
const ServiceLogs = () => {
  const [breadcrumbList] = useState([
    ...
    { name: 'Service Logs', pathname: '/servicelogs', active: true },
  ...
  const [incidentPanel, setIncidentPanel] = useState(false)
  const [servicelogPanel, setServicelogPanel] = useState(false)

  const { workorders } = useSelector((state) => state.workorder)

  const handleIncident = (e) => {
    e.preventDefault()
    setIncidentPanel(true)
  }

  const handleServicelog = (e) => {
    e.preventDefault()
    setServicelogPanel(true)
  }

  //   const fetchCompanyData = useCallback(() => dispatch(fetchWorkOrders()), [dispatch])

  useEffect(() => {
    document.title = 'Service Logs'
    //
    // fetchCompanyData()
  }, [])
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: 'Suplier Company',
        accessor: 'suplierCompany',
      },
      {
        Header: 'User',
        accessor: 'user',
  ...
  return (
    <div>
                ...
                <div className="customHeaderContainer">
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="cstCardbodyHeaderTitle">Service Logs</h3>
                    </div>
                  </div>
                </div>
                <div className="customHeaderToolbar">
                  <CButtonGroup>
                    <CButton
                      ...
                      active
                      className=" mr-2"
                      onClick={(e) => handleIncident(e)}
                    >
                      <i className="fa fa-plus  mr-2" /> Create Incident
                    </CButton>
                    <CButton color="primary" active onClick={(e) => handleServicelog(e)}>
                      <i className="fa fa-plus mr-2" /> Create Service Log
                    </CButton>
                  </CButtonGroup>
                </div>
              </div>
          ...
          <div className="space">
            ...
            <div>
              <button
                className="btn btn-close"
                aria-label="Close"
                onClick={(e) => setIncidentPanel(false)}
              ></button>
            </div>
          </div>
        }
      ...
      <SlidingPane
        className="cstSlidePanel"
        overlayClassName="cstSlidePanelOverlay"
        isOpen={servicelogPanel}
        title={
          <div className="space">
            <div>
              <span>New Service Logs</span>
            </div>
            <div>
              <button
                className="btn btn-close"
                aria-label="Close"
                onClick={(e) => setServicelogPanel(false)}
              ></button>
            </div>
          </div>
        }
