...
import NewCurrency from '../panels/NewCurrency'
...
const Table = ({ columns, data }) => {
  ...
  const { fetchingCurrencies } = useSelector((state) => state.currency)
      ...
      <div className="pageBoxSizing-container cst-tableResponsive">
        <div className="table-responsive table-truncate pageTableWrapper">
          <div>
            ...
            {/* table info */}
            {!fetchingCurrencies && !firstPageRows.length > 0 && (
              <div className="table-info">
...
const CurrencyList = () => {
  ...
  const fetchCurrencyData = useCallback(() => dispatch(fetchCurrencies()), [dispatch])
  const { currencies } = useSelector((state) => state.currency)

  const handleShowMore = useCallback((e) => {
    e.preventDefault()
    $('#moreVisible').toggleClass('hide')
  }, [])

  const handleDeleteItem = useCallback((e, items) => {
    e.preventDefault()
    $('.table-action-dropdown .dropdown-menu').removeClass('show')
  ...
  const handleEditItem = useCallback((e, item) => {
    e.preventDefault()
    console.log(item)
  }, [])
  ...
  const columns = useMemo(
          ...
          <div>
            <div className="table-action-dropdown">
              <CDropdown>
                ...
                <CDropdownMenu>
                  <CDropdownItem onClick={(e) => handleEditItem(e, original)}>
                    <i className="fa fa-edit" />
                    <span className="text-rl">Edit</span>
                  </CDropdownItem>
                  <CDropdownDivider></CDropdownDivider>
                  <div id="moreVisible" className="hide">
                    <CDropdownItem onClick={(e) => handleDeleteItem(e, original)}>
                      <i className="fa fa-trash" />
                      <span className="text-rl">Delete</span>
                    </CDropdownItem>
                  </div>
                  <CDropdownItem onClick={(e) => handleShowMore(e)}>
                    <span className="text-rl">Show More</span>
                  </CDropdownItem>
                </CDropdownMenu>
              </CDropdown>
            </div>
    ...
    [handleShowMore, handleDeleteItem, handleEditItem],
  )
  const data = useMemo(() => (currencies.length > 0 ? currencies : []), [currencies])
  ...
  return (
    <div>
          ...
          <div className="toolBarContainer">
            <div className="customHeaderContainer">
              <div className="customHeaderContainer-footer">
                <div className="card-title">
                  <h3 className="st-Title">Currencies</h3>
                </div>
              </div>
            </div>
            <div className="customHeaderToolbar">
              <CButtonGroup>
                ...
                >
                  <i className="fa fa-plus mr-2" /> New Currency
                </CButton>
              </CButtonGroup>
            </div>
          </div>
      ...
      >
        <div>
          <NewCurrency setToggleTransPanel={setToggleTransPanel} />
        </div>
        <div className="slide-pane__footer">
          <div className="float-right">
            <CButton color="secondary" variant="ghost" onClick={(e) => handleCancelSlide(e)}>
              Cancel
            </CButton>
          </div>
        </div>
