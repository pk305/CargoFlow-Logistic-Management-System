...
import NewInvoiceItem from '../panels/NewInvoiceItem'
import { destroyFinitem, fetchFinitems } from 'src/redux/slices/finitemSlice'
import { capitalize } from 'lodash'
import $ from 'jquery'
import AccIntergration from '../panels/AccIntergration'
...
const Table = ({ columns, data }) => {
  ...
  const { fetchingFinitems } = useSelector((state) => state.finitem)
...
const InvoiceItems = () => {
  ...
  const { finitems } = useSelector((state) => state.finitem)
  const [accIntergrationModal, setAccIntergrationModal] = useState(false)

  const closeAccIntgMod = () => {
    setAccIntergrationModal(false)
  }

  const handleAccIntgMod = useCallback((e, item) => {
    e.preventDefault()
    $('.dropdown-menu').removeClass('show')
    setAccIntergrationModal(true)
  }, [])

  const handleDelInvoiceItm = useCallback(
    (e, item) => {
      e.preventDefault()
      $('.dropdown-menu').removeClass('show')
      ...
      var n = new Noty({
        ...
        buttons: [
          Noty.button(
            ...
            'btn btn-default btn-sm del-bnt-mr text-danger float-right',
            async function () {
              const resData = await dispatch(destroyFinitem(item.id)).unwrap()
              if (resData) {
                new Noty({
                  text: 'Invoice Item has been deleted succesfully',
                  layout: 'topRight',
                  progressBar: false,
                  type: 'alert',
                  timeout: 3000,
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="table-action-dropdown">
              <CDropdown>
                ...
                <CDropdownMenu>
                  ...
                  <CDropdownItem href="#" onClick={(e) => handleAccIntgMod(e, original)}>
                    <i className="fa fa-link mr-2" />
                    <span>Intergration</span>
                  </CDropdownItem>
                  <CDropdownItem href="#" onClick={(e) => handleDelInvoiceItm(e, original)}>
      ...
      },
      {
        ...
        accessor: 'salesPrice',
        // eslint-disable-next-line react/prop-types
        Cell: ({ value }) => <span>{parseFloat(value) > 0 ? value : null}</span>,
      },
      {
      ...
      },
      {
        ...
        accessor: 'status',
        // eslint-disable-next-line react/prop-types
        Cell: ({ value }) => <span>{value ? capitalize(value) : null}</span>,
      },
    ],
    [handleAccIntgMod, handleDelInvoiceItm],
  )

  const data = useMemo(() => (finitems && finitems.length > 0 ? finitems : []), [finitems])

  const fetchInvItemData = useCallback(() => dispatch(fetchFinitems()), [dispatch])
  ...
  return (
    <div>
            ...
            <div className="customHeaderContainer">
              <div className="customHeaderContainer-footer">
                <div className="card-title">
                  <h3 className="st-Title">Invoice Items</h3>
                </div>
              </div>
            </div>
            <div className="customHeaderToolbar">
              <CButtonGroup>
                <CButton color="primary" active onClick={() => setInvItemModal(!invItemModal)}>
                  <i className="fa fa-plus mr-2" /> New Invoice Item
                </CButton>
              </CButtonGroup>
            </div>
          </div>
      ...
      >
        <div>
          <NewInvoiceItem setInvItemModal={setInvItemModal} />
        </div>
      </SlidingPane>
      ...
      >
        <div>
          <AccIntergration setAccIntergrationModal={setAccIntergrationModal} />
        </div>
      </SlidingPane>
    </div>
  )
