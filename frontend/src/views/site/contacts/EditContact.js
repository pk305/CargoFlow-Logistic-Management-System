...
import { clearContactError, findContact, updateContact } from 'src/redux/slices/contactSlice'
...
const EditContact = () => {
  ...
  const handleSubmitContact = async (e) => {
    e.preventDefault()
    const resData = await dispatch(updateContact({ Id: params.id, ...contactData })).unwrap()
    if (resData) {
      new Noty({
        type: 'alert',
        layout: 'topCenter',
      ...
      async function checkBooking() {
        ...
        if (resData) {
          setContactData(() => ({
            ...
            contactEmail: resData.email ? resData.email : '',
            companyId: resData.companyId ? resData.companyId : '',
            jobTitle: resData.jobTitle ? resData.jobTitle : '',
            gsm: resData.gsm ? resData.gsm : '',
            skype: resData.skype ? resData.skype : '',
            tel: resData.telephone ? resData.telephone : '',
            operationId: resData.operationId ? resData.operationId : '',
            roadNotify: resData.roadNotify ? resData.roadNotify : '',
            seaNotify: resData.seaNotify ? resData.seaNotify : '',
            airNotify: resData.airNotify ? resData.airNotify : '',
            railNotify: resData.railNotify ? resData.railNotify : '',
            customNotify: resData.customNotify ? resData.customNotify : '',
            depotNotify: resData.depotNotify ? resData.depotNotify : '',
            financeNotify: resData.financeNotify ? resData.financeNotify : '',
                ...
                <div className="customHeaderContainer">
                  ...
                  <div className="customHeaderContainer-footer">
                    <div className="card-title">
                      <h3 className="card-label">Edit Contact</h3>
                    </div>
                  </div>
                </div>
              </div>
