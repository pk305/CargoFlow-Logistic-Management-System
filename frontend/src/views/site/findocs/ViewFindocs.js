...
const ViewFindocs = () => {
  ...
  const [printModal] = useState(false)
  // const [setTempInvoiceId] = useState(null)
  // const [ setInvConfirm] = useState(false)
  ...
  //   (e) => {
  //     e.preventDefault()
  //     dispatch(findTemplate({ Id: showFindoc.id, type: 'invoice' }))
  //     // setTempInvoiceId(showFindoc.invoiceRefId)
  //     viewPrintModal(!printModal)
  //   },
  //   [printModal, dispatch, showFindoc],
  // )
  ...
  useEffect(() => {
    if (finId) {
      async function findFindocData() {
        const resData = await dispatch(findFindoc(finId)).unwrap()
        if (resData) {
          document.title = toUpper(resData.code)
          //   console.log(resData)
                      ...
                      <div className="customMiniBar-header">
                        <div className="minibar-left">
                          <span className="minbarTitle">
                            {toUpper(showFindoc && showFindoc.code)}
                          </span>
                          <div className="minbarSubtitle">
                            <h4>{showFindoc && showFindoc.account && showFindoc.account.name}</h4>
